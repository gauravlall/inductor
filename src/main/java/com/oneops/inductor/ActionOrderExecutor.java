...
import com.oneops.cms.util.Masker;
...
public class ActionOrderExecutor extends AbstractOrderExecutor {
    ...
    public Map<String, String> process(CmsWorkOrderSimpleBase o, String correlationId) throws IOException {
        ...
        Masker.maskSecuredFields(ao, CmsConstants.ACTION_ORDER_TYPE);
    ...
    private String getActionOrderHost(CmsActionOrderSimple ao, String logKey) {
        ...
        if (ao.getActionName().equalsIgnoreCase("user-custom-attachment")) {
            ...
            if (hostCi.getCiClassName() != null && hostCi.getCiClassName().equalsIgnoreCase("bom.Cluster")) {
                if (isAttributeNotEmpty(hostCi, SHARED_IP)) {
                    ...
                    if (env == null)
                        throw new IllegalArgumentException("ao for this action (" + ao.getActionName() + ") for ns " + ao.getNsPath());
                    ...
                    if (randomInt > -1)
                        digCmd = new String[]{"/usr/bin/dig", "+short", host, "@" + authoritativeDnsServers.get(randomInt)};
    ...
    private void writeChefRequestToFile(String fileName, Map<String, Object> chefRequest) throws IOException {
